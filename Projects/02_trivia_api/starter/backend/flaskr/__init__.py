...
def create_app(test_config=None):
    ...
    def delete_questions(question_id):
        ...
        affectedRowsCount = 0
        try:
            affectedRowsCount = Question.query.filter(
    ...
    def get_questions_of_category(cat_id):
        questions = []
        # if not declared here Would I be able to access it outside the try !!
        # ??
        category_type = None

        # get the category Type as string, also make sure that it exists.
        try:
            category_type = db.session.query(
        ...
        try:
            questions = db.session.query(Question).filter(
                Question.category == cat_id).order_by(
                Question.id).all()
        except Exception as e:
        ...
        return jsonify({
            ...
            # all questions or regardless of the category
    ...
    def quiz_next_question():
        ...
        try:
            questionsQuery = db.session.query(Question).filter(
                Question.id.notin_(prevQuestionsIds))
            if quizCatId != 0:  # not All category
                questionsQuery = questionsQuery.filter(
                    Question.category == quizCatId)
